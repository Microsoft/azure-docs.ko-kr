...
서비스에서 Azure AD 테넌트에서 발급 한 토큰을 받으면 페더레이션 메타 데이터 문서에 게시 된 서명 키를 사용 하 여 토큰의 서명 유효성을 검사 해야 합니다. 페더레이션 메타데이터는 테넌트를 토큰 서명에 사용하는 인증서의 공개 부분을 포함합니다. 인증서 원시 바이트는 `KeyDescriptor` 요소에 표시됩니다. 토큰 서명 인증서는 `use` 특성의 값이 `signing`인 경우의 서명에만 유효합니다.
