...
description: Microsoft id 플랫폼 엔드포인트에 대 한 관련 라이브러리, 소스 및 샘플 링크와 함께 호환 되는 클라이언트 라이브러리 및 서버 미들웨어 라이브러리
...
[Microsoft id 플랫폼 엔드포인트](active-directory-v2-compare.md) 은 업계 표준 OAuth 2.0 및 openid connect Connect 1.0 프로토콜을 지원 합니다. MSAL (Microsoft 인증 라이브러리)은 Microsoft id 플랫폼 엔드포인트에서 사용할 수 있도록 설계 되었습니다. OAuth 2.0 및 Openid connect Connect 1.0를 지 원하는 오픈 소스 라이브러리를 사용할 수도 있습니다.
...
Microsoft id 플랫폼 엔드포인트은 다음과 같은 두 가지 유형의 라이브러리와 함께 작동 합니다.
...
* **호환 가능**: microsoft는 기본 시나리오에서 이러한 라이브러리를 테스트 하 고 microsoft id 플랫폼 엔드포인트을 사용 하 고 있음을 확인 했습니다. Microsoft는 이러한 라이브러리에 대 한 픽스를 제공 하지 않으며 이러한 라이브러리에 대 한 검토를 수행 하지 않았습니다. 문제 및 기능 요청은 라이브러리의 오픈 소스 프로젝트로 리디렉션되어야 합니다.

Microsoft id 플랫폼 엔드포인트을 사용 하는 라이브러리 목록은 다음 섹션을 참조 하십시오.
...
모든 표준 호환 라이브러리의 경우 Microsoft id 플랫폼 엔드포인트을 사용할 수 있습니다. 지원 위치를 알아야 합니다.
...
* Microsoft id 플랫폼 엔드포인트이 OAuth 2.0 또는 Openid connect Connect 1.0과 호환 되지 않는 문제를 발견 [한 경우 지원 요청을 만듭니다](https://docs.microsoft.com/azure/azure-portal/supportability/how-to-create-azure-support-request) .

## <a name="related-content"></a>관련 콘텐츠

Microsoft id 플랫폼 엔드포인트에 대 한 자세한 내용은 [microsoft id 플랫폼 개요][AAD-App-Model-V2-Overview]를 참조 하세요.
