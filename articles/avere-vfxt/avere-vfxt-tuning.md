...
클라우드 NAS 또는 게이트웨이 시나리오에서 vFXT 클러스터는 클라우드 컨테이너에 대한 NAS 스타일의 액세스를 제공 합니다. VFXT 클러스터와 클라우드 저장소 간의 데이터 속도를 높이려면 담당자가 캐시에서 저장소 볼륨으로 데이터를 보다 적극적으로 푸시하는 설정을 변경 하는 것이 좋습니다. 예:
...
지원 직원에 게 이러한 최적화에 대해 문의 하려면 시스템에 대한 [도움말 보기](avere-vfxt-open-ticket.md)에 설명 된 절차를 따르십시오.
