...
Webhook를 사용하면 사후 처리 또는 사용자 지정 알림을 위해 Azure 경고 알림을 다른 시스템으로 라우팅할 수 있습니다. 예를 들어, 들어오는 웹 Request to Send SMS를 처리 하 고, 버그를 기록 하거나, 채팅 또는 메시징 서비스를 사용 하 여 팀에 알릴 수 있는 서비스로 경고를 라우팅합니다. Webhook URI는 올바른 HTTP 또는 HTTPS 엔드포인트 이어야 합니다.
