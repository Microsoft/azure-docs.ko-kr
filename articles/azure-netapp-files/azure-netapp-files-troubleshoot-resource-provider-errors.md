...
description: 일반적인 Azure NetApp Files 리소스 공급자 오류에 대한 원인, 해결 방법 및 해결 방법을 설명 합니다.
...
***개체 내보내기 정책에 대한 값 중복 오류가 발생 했습니다. 규칙 [RuleIndex].***
...
@No__t_0 부분은 Active Directory 연결에 대한 `ActiveDirectoryId` 속성에 입력 한 값입니다.
...
이 메시지는 `RuleIndex`, `AllowedClients`, `UnixReadOnly`, `UnixReadWrite`, `Nfsv3`, `Nfsv4`에 대한 필드에 오류가 있음을 나타냅니다.

* 원인:   
@No__t_0, `AllowedClients`, `UnixReadOnly`, `UnixReadWrite`, `Nfsv`3 및 `Nfsv4` 필드 중 하나 이상에 대한 입력 유효성 검사 요청이 실패 했습니다.
* 해결 방법:   
명령줄에서 필요한 모든 매개 변수 및 충돌 하지 않는 매개 변수를 설정 해야 합니다. 예를 들어 `UnixReadOnly`와 `UnixReadWrite` 매개 변수를 동시에 설정할 수 없습니다.
* 해결 방법:   
위의 솔루션을 참조 하십시오.

***Vlan {2} {1}에 대한 IP 범위 {0} 이미 사용 중입니다.***
...
* 원인:   
하나 이상의 속성에 대한 입력 유효성 검사 요청이 실패 했습니다.
* 해결 방법:   
...
***' @No__t_1 '에 대한 작업 결과 id를 찾을 수 없습니다.***
...
* 해결 방법:   
볼륨에 대한 내보내기 정책을 반드시 사용 해야 하는 것은 아닙니다. 정책 내보내기 규칙을 사용 하지 않아도 되는 경우 내보내기 정책을 완전히 생략할 수 있습니다.

***Active directory는 하나만 허용 됩니다.***

이 오류는 Active Directory 구성을 만들려고 하는데 해당 지역에 구독에 대한 구성이 이미 있는 경우에 발생 합니다. 또한 두 개 이상의 Active Directory 구성을 만들려고 하면 오류가 발생할 수 있습니다.
...
이 오류는 활성 구독 또는 리소스에 대한 작업을 사용할 수 없음을 나타냅니다.

* 원인:   
구독 또는 리소스에 대한 작업을 사용할 수 없습니다.
* 해결 방법:   
...
이 오류는 볼륨에 대한 `ProtocolType`를 업데이트 하거나 패치 하려고 할 때 발생 합니다.  ProtocolType을 변경 하는 작업은 지원 되지 않습니다.
...
이 오류는 변경할 수 없는 속성에 대한 값을 정의할 때 발생 합니다. 예를 들어 볼륨 ID는 변경할 수 없습니다.
...
* 해결 방법:   
맞춤법 오류에 대한 요청을 확인 하 여 올바르게 참조 되는지 확인 합니다.
* 해결 방법:   
위의 솔루션 섹션을 참조 하십시오.
...
이 오류는 계정에 대한 Active Directory 구성을 만들거나 업데이트할 때 발생 합니다.
...
이 오류는 새 볼륨을 만들려고 하지만 `subnetId` 서브넷에 대한 `resourceId` 되지 않은 경우에 발생 합니다.

* 원인:   
이 오류는 새 볼륨을 만들려고 하지만 `subnetId` 서브넷에 대한 `resourceId` 되지 않은 경우에 발생 합니다. 
* 해결 방법:   
@No__t_0에 대한 값을 확인 하 여 사용 되는 서브넷에 대한 `resourceId` 포함 되어 있는지 확인 합니다.
* 해결 방법:   
없음. 위의 솔루션을 참조 하십시오. 
...
Azure API는 Azure NetApp Files API를 사용 하 여 볼륨을 관리 합니다. 오류는 API에 대한 통신의 문제를 나타냅니다.
...
***알 수 없는 ' {propertyName} ' 속성에 대한 값이 수신 되었습니다.***
...
* 해결 방법:   
모든 속성 이름의 철자가 올바른지와 구독과 리소스에 대한 속성을 사용할 수 있는지 확인 합니다.
* 해결 방법:   
...
이 오류는 성공 상태가 아닌 풀에서 볼륨을 만들려고 할 때 발생 합니다. 어떤 이유로 인해 용량 풀에 대한 만들기 작업이 실패 했을 가능성이 높습니다.
