...
description: 이 문서에서는 온-프레미스 Windows server에 대한 시스템 상태 백업 문제를 해결 하는 방법에 대해 알아봅니다.
...
이 문서에서는 시스템 상태 백업을 사용 하는 동안 발생할 수 있는 문제에 대한 해결 방법을 설명 합니다.
...
로컬 시스템에 Windows가 설치 된 볼륨에 있는 **시스템 볼륨 정보** 폴더에 대한 모든 권한이 있는지 확인 합니다. 일반적으로이는 **C:\system 볼륨 정보**입니다. 위의 권한이 올바르게 설정 되지 않은 경우 Windows Server 백업이 실패할 수 있음
...
- 리소스 관리자 배포의 Windows 시스템 상태에 대한 자세한 내용은 [Windows Server 시스템 상태 백업](backup-azure-system-state.md) 을 참조 하세요.
