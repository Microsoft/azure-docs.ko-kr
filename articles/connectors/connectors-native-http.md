---
title: HTTP 및 HTTPS 엔드포인트 호출
description: Azure Logic Apps를 사용 하 여 HTTP 및 HTTPS 엔드포인트에 보내는 요청 보내기
services: logic-apps
...
---
# <a name="send-outgoing-calls-to-http-or-https-endpoints-by-using-azure-logic-apps"></a>Azure Logic Apps를 사용 하 여 HTTP 또는 HTTPS 엔드포인트에 보내는 호출 보내기

[Azure Logic Apps](../logic-apps/logic-apps-overview.md) 및 기본 제공 http 트리거 또는 작업을 사용 하 여 HTTP 또는 HTTPS 엔드포인트으로 요청을 정기적으로 전송 하는 자동화 된 작업 및 워크플로를 만들 수 있습니다. 들어오는 HTTP 또는 HTTPS 호출을 수신 하 고 대신 응답 하려면 기본 제공 [요청 트리거 또는 응답 작업](../connectors/connectors-native-reqres.md)을 사용 합니다.

예를 들어 지정 된 일정에 따라 해당 엔드포인트을 확인 하 여 웹 사이트에 대한 서비스 엔드포인트을 모니터링할 수 있습니다. 웹 사이트의 작동이 중단 되는 경우와 같이 해당 엔드포인트에서 특정 이벤트가 발생 하면 이벤트는 논리 앱의 워크플로를 트리거하고 지정 된 작업을 실행 합니다.

정기적으로 엔드포인트을 검사 하거나 *폴링하기* 위해 워크플로의 첫 단계로 HTTP 트리거를 사용할 수 있습니다. 각 검사에서 트리거는 엔드포인트에 호출 또는 *요청*을 전송합니다. 엔드포인트의 응답은 논리 앱의 워크플로가 실행될지 여부를 결정합니다. 트리거는 응답에서 논리 앱의 작업으로 모든 콘텐츠를 전달합니다.

원하는 경우 엔드포인트 호출을 위해 워크플로의 다른 단계로 HTTP 작업을 사용할 수 있습니다. 엔드포인트의 응답은 워크플로의 나머지 작업을 실행하는 방법을 결정합니다.

대상 엔드포인트의 기능을 기반으로 하는 HTTP 커넥터는 TLS (전송 계층 보안) 버전 1.0, 1.1 및 1.2을 지원 합니다. 가능 하면 지원 되는 가장 높은 버전을 사용 하 여 엔드포인트에 대한 Logic Apps 협상 합니다. 따라서 예를 들어 엔드포인트이 1.2을 지 원하는 경우 커넥터는 1.2를 먼저 사용 합니다. 그렇지 않으면 커넥터에서 지원 되는 가장 높은 다음 버전을 사용 합니다.

## <a name="prerequisites"></a>필수 조건

* Azure 구독 Azure 구독이 없는 경우 [체험 Azure 계정에 등록](https://azure.microsoft.com/free/)합니다.

* 호출 하려는 대상 엔드포인트의 URL입니다.
...
* 대상 엔드포인트을 호출 하려는 논리 앱입니다. HTTP 트리거를 시작 하려면 [빈 논리 앱을 만듭니다](../logic-apps/quickstart-create-first-logic-app-workflow.md). HTTP 작업을 사용 하려면 원하는 트리거를 사용 하 여 논리 앱을 시작 합니다. 이 예제에서는 첫 번째 단계로 HTTP 트리거를 사용 합니다.

## <a name="add-an-http-trigger"></a>HTTP 트리거 추가

이 기본 제공 트리거는 엔드포인트에 대해 지정 된 URL에 대한 HTTP 호출을 수행 하 고 응답을 반환 합니다.
...
1. 대상 엔드포인트에 대한 호출에 포함 하려는 [HTTP 트리거 매개 변수에](../logic-apps/logic-apps-workflow-actions-triggers.md#http-trigger) 대한 값을 제공 합니다. 트리거가 대상 엔드포인트을 확인 하는 빈도에 대한 되풀이를 설정 합니다.
...
이 기본 제공 작업은 엔드포인트에 대해 지정 된 URL에 대한 HTTP 호출을 수행 하 고 응답을 반환 합니다.
...
1. 대상 엔드포인트에 대한 호출에 포함 하려는 [HTTP 작업 매개 변수의](../logic-apps/logic-apps-workflow-actions-triggers.md#http-action) 값을 제공 합니다.
...
예를 들어 `multipart/form-data` 유형을 지 원하는 사이트의 API를 사용 하 여 Excel 파일에 대한 HTTP POST 요청을 웹 사이트로 보내는 논리 앱이 있다고 가정 합니다. 이 작업의 모양은 다음과 같습니다.
...
트리거 및 동작 매개 변수에 대한 자세한 내용은 다음 섹션을 참조 하십시오.
...
이 정보를 반환 하는 HTTP 트리거 또는 작업의 출력에 대한 자세한 내용은 다음과 같습니다.
