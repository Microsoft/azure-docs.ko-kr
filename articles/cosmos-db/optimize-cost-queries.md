...
쿼리의 요청 단위는 여러 가지 요인에 따라 달라집니다. 예를 들어, 로드/반환 되는 Azure Cosmos 항목의 수, 인덱스에 대한 조회 수, 쿼리 컴파일 시간 등에 대한 정보를 표시 합니다. Azure Cosmos DB는 동일한 데이터에 대해 실행될 경우 동일한 쿼리가 항상 반복 실행에서 동일한 요청 단위 수를 사용하도록 보장합니다. 쿼리 실행 메트릭을 사용하는 쿼리 프로필은 요청 단위가 소비되는 적절한 방식을 제시합니다.  
