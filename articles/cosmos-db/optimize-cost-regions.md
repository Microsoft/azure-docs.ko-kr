...
다중 마스터 시스템에서 쓰기 작업에 대해 사용 가능한 순 RU는 `N`배만큼 증가합니다. 여기서 `N`은 쓰기 지역 수입니다. 단일 지역 쓰기와 달리, 모든 지역은 이제 쓰기 가능하며 충돌 해결을 지원해야 합니다. 작성자의 워크로드 양도 증가했습니다. 비용 계획 관점에서 전 세계에서 쓰기에 대한 `M`를 수행 하려면 컨테이너 또는 데이터베이스 수준에서 M `RUs`를 프로 비전 해야 합니다. 원하는 수의 지역을 추가하고 쓰기에 사용하여 전 세계적으로 `M` RU에 상당하는 쓰기를 수행할 수 있습니다. 
