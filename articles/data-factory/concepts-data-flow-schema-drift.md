...
원본 정의에서 데이터 흐름으로 들어오는 열은 원본 프로젝션에 없는 경우 "데이터베이스가 드리프트"로 정의 됩니다. 원본 변환의 프로젝션 탭에서 원본 프로젝션을 볼 수 있습니다. 원본에 대한 데이터 집합을 선택 하면 ADF가 자동으로 데이터 집합에서 스키마를 가져와 해당 데이터 집합 스키마 정의에서 프로젝트를 만듭니다.
...
[데이터 흐름 식 언어](data-flow-expression-functions.md)에서 열 패턴에 대한 추가 기능 및 "byName" 및 "byPosition"를 포함 하는 스키마 드리프트를 찾을 수 있습니다.
