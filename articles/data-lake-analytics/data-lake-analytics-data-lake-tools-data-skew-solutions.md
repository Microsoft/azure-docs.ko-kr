...
앞의 예제에서 해당 국가/지역에 대한 세금 감사 작업만 확인 하려면 ID 번호를 키로 선택 하 여 데이터 배포를 향상 시킬 수 있습니다. 다른 파티션/배포 키를 선택하면 때때로 데이터를 보다 고르게 배포할 수 있지만 이로 인해 비즈니스 논리에 영향을 주지 않도록 해야 합니다. 예를 들어, 각 주의 세금 합계를 계산하려면 _주_를 파티션 키로 지정하는 것이 좋습니다. 이러한 문제가 계속되면 옵션 3을 사용해 보세요.
