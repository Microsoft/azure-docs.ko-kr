...
- API 엔드포인트: Datahub URL입니다 (예: https://\<datahub >).
- 테넌트 ID
- 클라이언트 ID
- 클라이언트 암호
- EventHub 연결 문자열
...
API 요청을 만들려면 HTTP (POST) 메서드와 API 서비스에 대 한 URL을 결합 하 고 요청에 대 한 쿼리, 데이터 전송, 생성 또는 삭제를 위해 리소스에 대 한 URI를 결합 합니다. 그런 다음 하나 이상의 HTTP 요청 헤더를 추가 합니다. API 서비스에 대 한 URL은 API 엔드포인트, 즉 Datahub URL (https://\<해당 하는 datahub >)입니다.  
