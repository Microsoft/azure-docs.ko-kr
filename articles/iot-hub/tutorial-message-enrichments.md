...
*메시지 강화* 메시지를 지정 된 엔드포인트으로 보내기 전에 추가 정보를 사용 하 여 메시지를 *스탬프* Azure IoT Hub 기능을 설명 합니다. 메시지 강화를 사용 하는 한 가지 이유는 다운스트림 처리를 간소화 하는 데 사용할 수 있는 데이터를 포함 하는 것입니다. 예를 들어 장치 쌍 태그를 사용 하는 장치 원격 분석 메시지를 보강 고객에 대 한 부하를 줄여이 정보에 대 한 장치 쌍 API 호출을 수행할 수 있습니다. 자세한 내용은 [메시지 강화 개요](iot-hub-message-enrichments-overview.md)를 참조 하세요.
...
메시지 라우팅 및 메시지 강화에 대 한 구성을 완료 한 후 응용 프로그램을 사용 하 여 IoT hub에 메시지를 보냅니다. 그런 다음 허브는 두 저장소 컨테이너에 모두 라우팅합니다. **보강** 저장소 컨테이너의 엔드포인트에 전송 된 메시지만 보강 됩니다.
...
Azure CLI 스크립트는 필요한 리소스를 만드는 것 외에도 별도의 저장소 컨테이너인 엔드포인트에 대 한 두 개의 경로도 구성 합니다. 메시지 라우팅을 구성 하는 방법에 대 한 자세한 내용은 [라우팅 자습서](tutorial-routing.md)를 참조 하십시오. 리소스를 설정한 후에는 [Azure Portal](https://portal.azure.com) 를 사용 하 여 각 엔드포인트에 대 한 메시지 강화 구성 합니다. 그런 다음 테스트 단계를 계속 진행 합니다.

> [!NOTE]
> 모든 메시지는 두 엔드포인트으로 모두 라우팅되고 구성 된 메시지 강화를 사용 하 여 엔드포인트으로 이동 하는 메시지만 보강 됩니다.
>
...
* 두 개의 서로 다른 저장소 계정에 대 한 라우팅을 설정 합니다.
    * 각 저장소 계정 컨테이너에 대 한 엔드포인트을 만듭니다.
    * 각 저장소 계정 컨테이너 엔드포인트에 대 한 경로를 만듭니다.
...
1. **리소스 그룹**을 선택 하 여 IoT hub로 이동 합니다. 그런 다음이 자습서에 대해 설정 된 리소스 그룹을 선택 합니다 (**ContosoResourcesMsgEn**). 목록에서 IoT hub를 찾아 선택 합니다. IoT hub에 대 한 **메시지 라우팅** 을 선택 합니다.
   ...
   메시지 라우팅 창에는 **경로**, **사용자 지정 엔드포인트**및 **보강 메시지**라는 세 개의 탭이 있습니다. 처음 두 탭을 탐색 하 여 스크립트에 의해 설정 된 구성을 확인 합니다. 세 번째 탭을 사용 하 여 메시지 강화를 추가 합니다. **보강**라는 저장소 컨테이너의 엔드포인트으로 이동 하는 메시지를 보강 하겠습니다. 이름 및 값을 입력 한 다음 드롭다운 목록에서 **ContosoStorageEndpointEnriched** 엔드포인트을 선택 합니다. 메시지에 IoT hub 이름을 추가 하는 보강을 설정 하는 방법의 예는 다음과 같습니다.

   ![첫 번째 보강 추가](./media/tutorial-message-enrichments/add-message-enrichments.png)

2. 이러한 값을 ContosoStorageEndpointEnriched 엔드포인트 목록에 추가 합니다.
...
이제 엔드포인트에 대해 강화 메시지를 구성 했으므로 시뮬레이션 된 장치 응용 프로그램을 실행 하 여 IoT hub로 메시지를 보냅니다. 허브는 다음 작업을 수행 하는 설정으로 설정 되었습니다.

* 저장소 엔드포인트 ContosoStorageEndpointOriginal로 라우팅되는 메시지는 보강 되지 않으며 저장소 컨테이너 `original`에 저장 됩니다.

* 저장소 엔드포인트 ContosoStorageEndpointEnriched로 라우팅되는 메시지는 보강 되 고 저장소 컨테이너 `enriched`저장 됩니다.
...
앱은 1초마다 새로운 디바이스-클라우드 메시지를 IoT Hub에 보냅니다. 메시지에는 디바이스 ID, 온도, 습도 및 메시지 수준(기본값이 `normal`)과 함께 JSON 직렬화된 개체가 포함됩니다. `critical` 또는 `storage`의 수준을 임의로 할당 하므로 메시지는 저장소 계정 또는 기본 엔드포인트으로 라우팅됩니다. 저장소 계정의 **보강** 컨테이너로 전송 되는 메시지는 보강 됩니다.
...
Unenriched 메시지는 다음과 같습니다. 이러한 필드는 강화에 의해 추가 되기 때문에 "my IoT Hub," "devicelocation" 및 "customerID"는 여기에 표시 되지 않습니다. 이 엔드포인트에는 강화 없습니다.
...
* [IoT Hub 메시지 라우팅을 사용 하 여 다른 엔드포인트으로 장치-클라우드 메시지 보내기](iot-hub-devguide-messages-d2c.md)
