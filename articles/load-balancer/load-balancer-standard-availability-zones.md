...
또한 각 영역 내의 부하 분산 된 엔드포인트에 대해 영역 프런트 엔드를 직접 사용할 수 있습니다. 또한 이를 통해 부하 분산된 영역별 엔드포인트를 공개하여 각 영역을 개별적으로 모니터링할 수도 있습니다.  또는 공용 엔드포인트의 경우 [Traffic Manager](../traffic-manager/traffic-manager-overview.md) 와 같은 dns 부하 분산 제품과 통합 하 고 단일 dns 이름을 사용할 수 있습니다. 그러면 클라이언트는이 DNS 이름을 여러 영역 IP 주소로 확인 합니다.  
