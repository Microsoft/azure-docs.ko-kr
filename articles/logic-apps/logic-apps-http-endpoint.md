...
description: Azure Logic Apps에서 논리 앱 워크플로를 호출, 트리거 또는 중첩 하기 위해 HTTP 엔드포인트을 설정 합니다.
services: logic-apps
...
---
# <a name="call-trigger-or-nest-logic-apps-by-using-http-endpoints-in-azure-logic-apps"></a>Azure Logic Apps에서 HTTP 엔드포인트을 사용 하 여 논리 앱 호출, 트리거 또는 중첩

논리 앱이 다른 서비스에서 들어오는 요청을 받을 수 있도록 URL을 통해 논리 앱을 호출할 수 있도록 하려면 기본적으로 동기 HTTP 엔드포인트을 해당 논리 앱의 트리거로 제공 하면 됩니다. 이 기능을 설정 하면 논리 앱을 다른 논리 앱 내에 중첩 하 여 호출 가능 엔드포인트의 패턴을 만들 수도 있습니다.

HTTP 엔드포인트을 설정 하기 위해 논리 앱에서 들어오는 요청을 받을 수 있도록 하는 이러한 트리거 형식 중 하나를 사용할 수 있습니다.
...
* HTTP 엔드포인트을 트리거로 설정 하려는 논리 앱입니다. 빈 논리 앱 또는 현재 트리거를 바꾸려는 기존 논리 앱으로 시작할 수 있습니다. 이 예에서는 빈 논리 앱으로 시작 합니다.

## <a name="create-a-callable-endpoint"></a>호출 가능 엔드포인트 만들기
...
1. 논리 앱을 저장합니다.
   ...
   논리 앱의 **개요** 창에서 HTTP 엔드포인트 URL을 가져올 수도 있습니다.
   ...
   1. **콜백 url [POST]** 에서 url을 복사 합니다.
      ...
      ![Azure Portal에서 HTTP 엔드포인트 URL을 복사 합니다.](./media/logic-apps-http-endpoint/copy-manual-trigger-callback-url.png)
...
1. **메서드** 목록에서 instead of 트리거를 필요로 하는 다른 메서드를 선택 합니다. 또는 사용자 지정 메서드를 지정할 수 있습니다.
   ...
   예를 들어 나중에 HTTP 엔드포인트의 URL을 테스트할 수 있도록 **GET** 메서드를 선택 합니다.

   ![트리거에 사용할 HTTP 메서드를 선택 합니다.](./media/logic-apps-http-endpoint/select-method-request-trigger.png)

## <a name="accept-parameters-in-endpoint-url"></a>엔드포인트 URL의 매개 변수 허용

엔드포인트 URL에 매개 변수를 허용 하려면 트리거에서 상대 경로를 지정 합니다. 또한 HTTP 요청에 필요한 [메서드](#set-method) 를 명시적으로 설정 해야 합니다.
...
## <a name="call-logic-app-through-http-endpoint"></a>HTTP 엔드포인트을 통해 논리 앱 호출

HTTP 엔드포인트을 만든 후 엔드포인트의 전체 URL에 HTTP `POST` 요청을 전송 하 여 논리 앱을 트리거할 수 있습니다. Logic Apps는 직접 액세스 엔드포인트에 대한 기본 제공 지원을 포함합니다.
...
호출자에 게 콘텐츠를 반환 하 여 논리 앱을 트리거하는 특정 요청에 응답 하려는 경우가 있습니다. 응답의 상태 코드, 헤더 및 본문을 구성 하려면 응답 작업을 사용 합니다. 이 작업은 워크플로의 끝뿐만 아니라 논리 앱의 어디서나 나타날 수 있습니다. 논리 앱에 응답 작업이 포함 되어 있지 않은 경우 HTTP 엔드포인트은 **202 수락** 상태를 사용 하 여 *즉시* 응답 합니다.
...
**A**: 예, HTTP 엔드포인트은 [Azure API Management](../api-management/api-management-key-concepts.md)를 통해 더 많은 고급 구성을 지원 합니다. 또한 이 서비스는 Logic Apps를 포함한 모든 API를 일관성 있게 관리하고 사용자 지정 도메인 이름을 설정하고 다음과 같은 더 많은 인증 방법을 사용하는 기능을 제공합니다.
