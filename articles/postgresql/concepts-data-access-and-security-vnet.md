...
description: Vnet (virtual network) 서비스 엔드포인트을 사용 하 여 Azure Database for PostgreSQL 단일 서버에 연결 하는 방법을 알아봅니다.
author: rachel-msft
ms.author: raagyema
...
---
# <a name="use-virtual-network-service-endpoints-and-rules-for-azure-database-for-postgresql---single-server"></a>Azure Database for PostgreSQL 단일 서버에 대 한 Virtual Network 서비스 엔드포인트 및 규칙 사용
...
가상 네트워크 규칙을 만들려면 먼저 VNet ( [가상 네트워크][vm-virtual-network-overview] ) 및 참조할 규칙의 [가상 네트워크 서비스 엔드포인트][vm-virtual-network-service-endpoints-overview-649d] 이 있어야 합니다. 다음 그림에서는 Virtual Network 서비스 엔드포인트가 Azure Database for PostgreSQL에서 작동하는 방법을 보여줍니다.
