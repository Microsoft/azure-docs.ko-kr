...
| **컴퓨터 신뢰 경계** | <ul><li>[디바이스에서 조직 정책에 따라 구성된 엔드포인트 보안 제어를 사용하는지 확인](#controls-policies)</li></ul> |
...
## <a id="controls-policies"></a>디바이스에서 조직 정책에 따라 구성된 엔드포인트 보안 제어를 사용하는지 확인
...
| **특성**              | N/A  |
| **참조**              | N/A  |
| **단계** | 디바이스에 디스크 수준 암호화를 위한 bit-locker, 업데이트된 서명이 있는 바이러스 백신, 호스트 기반 방화벽, OS 업그레이드, 그룹 정책 등과 같은 엔드포인트 보안 제어 기능이 조직의 보안 정책에 따라 구성되어 있는지 확인합니다. |
