...
1. **개요** 창에 **테스트 엔드포인트이**표시 됩니다. 새 브라우저 창에 복사 하 여 붙여넣고 새 PiggyMetrics 페이지가 표시 됩니다.

>[!TIP]
> * 테스트 엔드포인트이 슬래시 (/)로 끝나는지 확인 하 여 CSS 파일이 올바르게 로드 되는지 확인 합니다.  
> * 브라우저에서 페이지를 보기 위해 로그인 자격 증명을 입력해야 하는 경우, [URL 디코드](https://www.urldecoder.org/)를 사용하여 테스트 엔드포인트를 디코드합니다. URL 디코드는 "https://\<username>:\<password>@\<cluster-name>.test.azureapps.io/gateway/green" 형식의 URL을 반환합니다.  이 형식을 사용 하 여 엔드포인트에 액세스 합니다.
