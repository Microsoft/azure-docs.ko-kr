...
지역 B에서 중단이 발생한 경우 트래픽 관리자가 지역 B의 엔드포인트 웹 앱 2 실패를 탐지하고 성능 저하되었다고 표시합니다(1). 그 동안 장애 조치 그룹이 읽기 전용 수신기를 지역 A로 전환합니다(2). 이 중단은 최종 사용자 환경에 영향을 미치지 않으나 중단 기간 중 주 데이터베이스가 노출됩니다. 다음 다이어그램은 보조 지역에서의 실패를 보여 줍니다.
...
* 읽기 전용 워크로드는 각 지역의 다른 엔드포인트에 연결해야 합니다.
