...
* **저장소 데이터 암호화** 사설 클라우드의 vSAN 저장소에 상주 하는 미사용 데이터를 암호화할 수 있습니다. vSAN은 Azure vNet 또는 온-프레미스 환경에 배포할 수 있는 외부 키 관리 서버를 지원 합니다.  자세한 내용은 [CloudSimple 사설 클라우드에 대 한 vSAN 암호화 구성](vsan-encryption.md)을 참조 하세요.
